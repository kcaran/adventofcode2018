...
print "($unit->{ y }, $unit->{ x }) = $unit->{ hp }\n";
    ...
    return "The score is $score * $self->{ num_rounds } = " . $score * $self->{ num_rounds } . "\n";
   }
...
print "($targets[0]->{ y }, $targets[0]->{ x }) : $targets[0]->{ hp }\n";
  ...
  sub move_unit {
    ...
    if ($enemy eq 'E') {
      return $self->end_battle( 'G' ) unless (grep { $_->{ hp } > 0 } @{ $self->{ elves } });
     }
    else {
      return $self->end_battle( 'E' ) unless (grep { $_->{ hp } > 0 } @{ $self->{ goblins } });
     }
  ...
  sub round {
    ...
    for my $unit (sort { $a->{ y } <=> $b->{ y } || $a->{ x } <=> $b->{ x } } (@{ $self->{ elves } }, @{ $self->{ goblins } })) {
      ...
      my $end_battle = $self->move_unit( $unit );
      return $end_battle if ($end_battle);
  ...
  sub new {
    my ($class, $input_file, $elf_help) = @_;
    my $self = {
     map => [],
     elves => [],
     goblins => [],
    ...
    for my $row ( Path::Tiny::path( $input_file )->lines_utf8( { chomp => 1 } )) {
      ...
      for my $col (split( '', $row )) {
        $self->{ map }[$y][$x] = $col;
        if ($col eq 'E') {
          push @{ $self->{ elves } }, Unit->new( $col, $x, $y, 3 + $elf_help );
         }
        if ($col eq 'G') {
          push @{ $self->{ goblins } }, Unit->new( $col, $x, $y, 3 );
         }
        $x++;
       }
      $y++;
...
my $map;
do {
  $map = Map->new( $input_file, $try );
  $map->print_map();
  while (!$end && $map->{ num_elves } == @{ $map->{ elves } }) {
    $end = $map->round();
    $map->print_map();
   }
  $try++ if (!$end);
} while ($map->{ num_elves } != @{ $map->{ elves } });

print "Elves need ", 3 + $try, " to $end\n";
exit;
